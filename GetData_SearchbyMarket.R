...
# Here we are collecting data by calling the API for each market and counting the industries in that market
# Instead of searching all of Canada, we only search the markets selected in imported file plus a 5 km buffer
# Imported market file should have markets defined by a shape file and include market characteristics desired
...
#convert lat long from shapefile to data frame
...
####--------------------------------------------------- Call API ----------------------####-----
# In order to only scrape the API once, we have commented out the API call
# For rerunning this file, we just load in the previously scraped data 
...
# convert API return from nasty list into df
...
#drop points outside Canadian provinces (important for border towns)

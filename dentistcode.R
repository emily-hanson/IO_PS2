...
nVar <- 'ndentist_1kmBuffer'
alternate_nVar <- c("ndentist_2kmBuffer", "ndentist_5kmBuffer")
...
zVars <- c('C1_COUNT_TOTAL_65.years.and.over','C1_COUNT_TOTAL_Average.number.of.children.in.census.families.with.children',
	'C1_COUNT_TOTAL_Average.total.income.in.2020.among.recipients....','C1_COUNT_TOTAL_Edatleasths','C1_COUNT_TOTAL_Unemployed')
#demand and cost shifting vars 
wVars <- c("C1_COUNT_TOTAL_Dwellingperkm","C1_COUNT_TOTAL_Median.value.of.dwellings....",
	'AT','QC','ON','BC')
...
#figure 2
figure2Data <- dfOg%>%
  count(popRange = round(2*get(popVar))/2)
...
#figure 3
figure3Data <- dfOg%>%
  count(popRange = round(get(popVar)),
